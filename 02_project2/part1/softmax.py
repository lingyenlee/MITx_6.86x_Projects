...
def compute_probabilities(X, theta, temp_parameter):
    ...
    H = (1 / np.sum(ex_diff, axis=0) ) * ex_diff
    return H
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    no_of_labels = theta.shape[0]
    probabilities = compute_probabilities(X, theta, temp_parameter)
    reg_term = lambda_factor/2 * np.square(theta).sum()
   
    sum_prob = 0
    for i in range(no_of_samples):
        for j in range(no_of_labels):
            if Y[i] == j:
                sum_prob += np.log(probabilities[j][i])
    
    cost = (- 1/no_of_samples) * sum_prob + reg_term
    return cost
    raise NotImplementedError
